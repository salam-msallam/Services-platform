...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <h1 class="text-2xl font-semibold text-slate-900">{{ __('admin.categories') }}</h1>
            <a
                ...
                <table class="min-w-full text-sm">
                    ...
                    @forelse($categories as $category)
                        ...
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="px-4 py-3 font-medium text-slate-900">{{ $category->getTranslation('name', 'ar') }}</td>
                            <td class="px-4 py-3 font-medium text-slate-900">{{ $category->getTranslation('name', 'en') }}</td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ route('admin.categories.show', $category) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-semibold">
                                        {{ __('admin.view_details') }}
                                    </a>
                                    <a href="{{ route('admin.categories.edit', $category) }}" class="inline-flex items-center px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-semibold">
                                        {{ __('admin.edit') }}
                                    </a>
